...
DvG_NeoPixel_Effects::DvG_NeoPixel_Effects(Adafruit_NeoPixel_ZeroDMA* thisStrip) {
  strip = thisStrip;
  effect_is_done = true;
}
#else
...
bool DvG_NeoPixel_Effects::effectIsDone(void) {
  return effect_is_done;
}
...
uint8_t DvG_NeoPixel_Effects::red(uint32_t c) {
  return (c >> 16);
}

uint8_t DvG_NeoPixel_Effects::green(uint32_t c) {
  return (c >> 8);
}

uint8_t DvG_NeoPixel_Effects::blue(uint32_t c) {
  return (c);
}
...
void DvG_NeoPixel_Effects::holdAndWait(uint32_t wait) {
  now = millis();
  if (effect_is_done) {startup();}
  if (now > last_update + wait) {finish();}
}
...
void DvG_NeoPixel_Effects::rainbowSpatial(uint16_t wait, uint8_t num_cycles) {
  ...
  if (effect_is_done | (now > last_update + wait)) {
    startup();
    for (iPx = 0; iPx < strip->numPixels(); iPx++) {
      strip->setPixelColor(iPx, Wheel(((iPx * 256 / strip->numPixels()) + j) & 255));
    }
    strip->show();

    j++;
    if (j == 256 * num_cycles) {finish();}
  }
}
